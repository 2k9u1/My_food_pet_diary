...
로그인 화면에 **비밀번호 칸이 있습니다** — 구글 로그인 팝업이 아니라 평범한 폼입니다. **버튼 하나로 가입과 로그인을 모두 처리**합니다: 처음 입력하면 그 비밀번호로 계정이 만들어지고, 다음에 같은 정보 + 같은 비밀번호로 다시 들어오면 이전 계정 그대로 로그인됩니다. 그래서 로그아웃하거나 기기를 바꿔도 **같은 정보 + 같은 비밀번호면 펫과 기록이 그대로 이어집니다.**

- **교사**: 이름 + 이메일 + 비밀번호. 실제 이메일을 입력합니다.
- **학생**: 이름(또는 별명) + 학급 코드 + 학번 + 비밀번호. 이메일은 몰라도 됩니다 — 화면에 이메일 칸 자체가 없습니다. (내부적으로 "학번+학급코드"로 만든, 실제로 존재하지 않아도 되는 주소를 Firebase에 등록하는 데만 씁니다.)
- **역할은 화면에서 본인이 직접 고릅니다.** "미리 등록된 이메일이어야 교사가 될 수 있음" 같은 검증은 없습니다. 대신 **학급 코드**가 실질적인 경계 역할을 합니다: 교사는 로그인하면 자기만의 학급 코드가 생기고, 그 코드를 아는 학생만 그 교사의 학급에 들어갈 수 있습니다. 학생이 "교사로 시작"을 누르더라도 자기만의 새 빈 학급이 생길 뿐, 실제 다른 학생 데이터에는 접근할 수 없습니다.
- **비밀번호를 잊어버리면 복구할 방법이 없습니다** (특히 학생 — 진짜 이메일이 없어서 "비밀번호 재설정 메일"을 보낼 수 없습니다). 학생에게는 비밀번호를 적어두거나 기억하기 쉬운 것으로 정하도록 안내해 주세요. 이 부분은 필요하면 이후에 "교사가 학생 비밀번호를 초기화해주는 기능"으로 보완할 수 있습니다.
...
src/
  ...
  lib/
    auth.ts                로그인 파사드 — Firebase 이메일/비밀번호 로그인 / 간편 로그인 중 하나를 자동 선택
...
2. 왼쪽 메뉴 **Authentication** → "시작하기" → 로그인 방법 탭에서 **이메일/비밀번호** 사용 설정. (Google/익명 로그인은 이제 쓰지 않습니다 — 켜져 있어도 상관없지만 안 켜도 됩니다.)
...
1. Vercel 배포 주소를 브라우저로 열고 "교사로 시작"을 눌러 이름/이메일/비밀번호를 입력 → 바로 교사 대시보드로 들어가면 성공입니다. 화면 오른쪽 위에 **학급 코드**(예: `AB12CD`)가 자동으로 생성되어 보입니다 — 이 코드를 학생들에게 알려주세요.
2. 다른 브라우저(또는 시크릿 창)에서 "학생으로 시작"을 누르면 학급 코드 + 이름/학번/비밀번호를 입력하는 화면이 나옵니다. 방금 확인한 학급 코드를 입력하고 제출하면 그 교사의 학급 학생으로 등록됩니다.
