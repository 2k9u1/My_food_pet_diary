...
import { loginStudent, loginTeacher } from "../lib/store";
...
export function LoginScreen({ onLogin }: { onLogin: (user: User) => void }) {
  ...
  async function handleSubmit() {
    ...
    try {
      const user =
        role === "student"
          ? await loginStudent({ displayName: name.trim(), studentNumber: studentNumber.trim(), useAlias })
          : await loginTeacher({ displayName: name.trim() });
      onLogin(user);
    } catch {
    ...
    <div className="login-wrap">
      <div className="card login-card stack">
        ...
        <p className="hint" style={{ textAlign: "center" }}>
          지금은 시연용 간편 로그인입니다. 실제 서비스에서는 이 화면이 구글 로그인으로 바뀝니다.
        </p>
      </div>
    </div>
  );
